...
def to_mod_list(employee_list):
   new_li = list(map(mod, employee_list))
...
def generate_usernames(mod_list):
   ...
   [new_li.append(a.replace(" ", "_")) for a in mod_list]
   #for a in mod_list:
   #   new_li.append(a.replace("_", " "))
...
def map_id_to_initial(employee_list):
   ...
   employee_dict = {a['name'][0] : a["id"] for a in employee_list}
   #for a in employee_list:
   #   employee_dict[a['name'][0]] = a["id"]
